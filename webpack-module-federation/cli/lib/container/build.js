...
module.exports = (options)=>{
  // webpack --config ./node_modules/@alsi/micro-app-webpack-config/container2/webpack.config.production.cjs && cp manifest.json dist && cp package.json dist
  const webpackConfigDir = require.resolve('@alsi/micro-app-webpack-config').replace(`${path.sep}index.js`,'')
  const webpackConfigPath = path.join(webpackConfigDir, 'container2', 'webpack.config.production.cjs')
