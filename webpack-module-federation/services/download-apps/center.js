...
module.exports = (minioClient, apps = []) => {
  ...
  const getAppVersion = (app) => new Promise((resolve, reject) => {
    const prefix = `app/${app.name}/${app.version}`;
    var data = [];
    ...
    stream.on('end', function (obj) {
      const versions = semverRsort(data.map(item => item.name.match(new RegExp(`app\/${app.name}\/(v.*)\/dist.zip`))[1]));
  ...
  const download = (app) => new Promise((resolve, reject) => {
    ...
    const objectName = `app/${app.name}/${app.version}/dist.zip`;
